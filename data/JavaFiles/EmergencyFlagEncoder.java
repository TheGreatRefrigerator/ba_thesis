...
public class EmergencyFlagEncoder extends
		ORSAbstractFlagEncoder {
	protected final HashSet<String> forwardKeys = new HashSet<String>(
			5);
	protected final HashSet<String> backwardKeys = new HashSet<String>(
			5);
	protected final HashSet<String> noValues = new HashSet<String>(
			5);
	protected final HashSet<String> yesValues = new HashSet<String>(
			5);
	protected final List<String> hgvAccess = new ArrayList<String>(
			5);

	// This value determines the maximal possible on roads
	// with bad surfaces
	protected int badSurfaceSpeed;

	// This value determines the speed for roads with
	// access=destination
	protected int destinationSpeed;

	/**
	 * A map which associates string to speed. Get some
	 * impression:
	...
	public EmergencyFlagEncoder(PMap properties) {
		this(properties.getInt("speed_bits", 5), properties
				.getDouble("speed_factor", 5), properties
						.getBool("turn_costs", false) ? 3
								: 0);

		setBlockFords(false);
	}

	public EmergencyFlagEncoder(int speedBits,
			double speedFactor, int maxTurnCosts) {
		super(speedBits, speedFactor, maxTurnCosts);
		restrictions.addAll(Arrays.asList("motorcar",
		...
		hgvAccess.addAll(Arrays.asList("hgv", "goods",
				"bus", "agricultural", "forestry",
				"delivery"));
		...
		trackTypeSpeedMap.put("grade2", 15); // now unpaved
												...
												// mixed
												// with ...
		trackTypeSpeedMap.put("grade3", 15); // ... hard and
												// soft
												// materials
		trackTypeSpeedMap.put("grade4", 10); // ... some
												// hard or
												// compressed
												// materials
		trackTypeSpeedMap.put("grade5", 5); // ... no hard
											// materials.
											// soil/sand/grass
		...
		_speedLimitHandler = new SpeedLimitHandler(this
				.toString(), defaultSpeedMap,
				badSurfaceSpeedMap, trackTypeSpeedMap);
	...
	/**
	 * Define the place of the speedBits in the edge flags
	 * for car.
	 */
	@Override
	public int defineWayBits(int index, int shift) {
		// first two bits are reserved for route handling in
		// superclass
		shift = super.defineWayBits(index, shift);
		speedEncoder = new EncodedDoubleValue("Speed",
				shift, speedBits, speedFactor,
				_speedLimitHandler.getSpeed("secondary"),
				maxPossibleSpeed);
	...
	@Override
	protected double applyMaxSpeed(ReaderWay way,
			double speed) {
	...
	public double getMaxSpeed(ReaderWay way) // runge
	{
		 ...
		 * if (maxspeedTag == null) { maxspeedTag =
		 * way.getTag("maxspeed"); bCheckMaxSpeed = true; }
		 * 
		 * double maxSpeed = parseSpeed(maxspeedTag);
		 * 
		 * double fwdSpeed =
		 * parseSpeed(way.getTag("maxspeed:forward")); if
		 * (fwdSpeed >= 0 && (maxSpeed < 0 || fwdSpeed <
		 * maxSpeed)) maxSpeed = fwdSpeed;
		 * 
		 * double backSpeed =
		 * parseSpeed(way.getTag("maxspeed:backward")); if
		 * (backSpeed >= 0 && (maxSpeed < 0 || backSpeed <
		...
		if (bCheckMaxSpeed) {
			...
			double defaultSpeed = _speedLimitHandler
					.getSpeed(highway);
	...
	protected double getSpeed(ReaderWay way) {
		...
		if (!Helper.isEmpty(highwayValue) && way.hasTag(
				"motorroad", "yes")
				&& highwayValue != "motorway"
				&& highwayValue != "motorway_link") {
			highwayValue = "motorroad";
		}
		Integer speed = _speedLimitHandler.getSpeed(
				highwayValue);
		if (speed == null)
		...
		if (highwayValue.equals("track")) {
			...
			if (!Helper.isEmpty(tt)) {
				Integer tInt = _speedLimitHandler
						.getTrackTypeSpeed(tt); // FIXME
				if (tInt != null && tInt != -1)
					speed = tInt;
			}
		}
		...
		/*
		 * if (way.hasTag("access")) // Runge
		 * //https://www.openstreetmap.org/way/132312559 {
		 * String accessTag = way.getTag("access"); if
		 * ("destination".equals(accessTag)) return 1; }
		 */
	...
	public long acceptWay(ReaderWay way) {
		...
		if (highwayValue == null) {
			if (way.hasTag("route", ferries)) {
				...
				if (motorcarTag == null)
					motorcarTag = way.getTag(
							"motor_vehicle");

				if (motorcarTag == null && !way.hasTag(
						"foot") && !way.hasTag("bicycle")
		...
		// {
		// String tt = way.getTag("tracktype");
		// if (tt != null && !tt.equals("grade1")) // TODO
		// allow higher grade values for forestry and
		// agriculture
		// return 0;

		// if (tt != null &&
		// !trackTypeSpeedMap.containsKey(tt))
		// return 0;
		// }
		...
		if (way.hasTag("impassable", "yes") || way.hasTag(
				"status", "impassable"))
			return 0;

		// do not drive street cars into fords
		// boolean carsAllowed = way.hasTag(restrictions,
		// intendedValues);
		// if (isBlockFords() &&
		// ("ford".equals(highwayValue) ||
		// way.hasTag("ford")) && !carsAllowed)
		// return 0;
		if (isBlockFords() && ("ford".equals(highwayValue)
				|| way.hasTag("ford")))
			return 0;
		...
		// do not drive cars over railways (sometimes
		// incorrectly mapped!)
		/*
		 * if (way.hasTag("railway") &&
		 * !way.hasTag("railway", acceptedRailways)) { //
		 * Runge, see
		 * http://www.openstreetmap.org/way/36106092 String
		 * motorcarTag = way.getTag("motorcar"); if
		 * (motorcarTag == null) motorcarTag =
		 * way.getTag("motor_vehicle");
		 * 
		 * if (motorcarTag == null ||
		 * "no".equals(motorcarTag)) return 0; }
		 */
	...
	@Override
	public long handleRelationTags(ReaderRelation relation,
			long oldRelationFlags) {
		return oldRelationFlags;
	}

	@Override
	public long handleWayTags(ReaderWay way, long allowed,
			long relationFlags) {
		if (!isAccept(allowed))
			return 0;
		...
		if (!isFerry(allowed)) {
			...
			boolean isRoundabout = way.hasTag("junction",
					"roundabout");
			if (isRoundabout)
			...
			double ferrySpeed = getFerrySpeed(way,
					_speedLimitHandler.getSpeed(
							"living_street"),
					_speedLimitHandler.getSpeed("service"),
					_speedLimitHandler.getSpeed(
							"residential"));
		...
		/*
		 * for (String restriction : restrictions) { if
		 * (way.hasTag(restriction, "destination")) { //
		 * This is problematic as Speed != Time flags =
		 * this.speedEncoder.setDoubleValue(flags,
		 * destinationSpeed); } }
		 */
		return flags;
	}
	 ...
	 * @param speed:
	 *            speed guessed e.g. from the road type or
	 *            other tags
	 * @return The assumed speed
	 */
	protected double applyBadSurfaceSpeed(ReaderWay way,
			double speed) {
		// limit speed if bad surface
		// if (badSurfaceSpeed > 0 && speed >
		// badSurfaceSpeed && way.hasTag("surface",
		// badSurfaceSpeedMap))
		// speed = badSurfaceSpeed;
		String surface = way.getTag("surface");
		if (surface != null) {
			Integer surfaceSpeed = _speedLimitHandler
					.getSurfaceSpeed(surface);
	...
	protected boolean isBackwardOneway(ReaderWay way) {
		return way.hasTag("oneway", "-1") || way.hasTag(
				"vehicle:forward", "no") || way.hasTag(
	...
	protected boolean isOneway(ReaderWay way) {
		return way.hasTag("oneway", oneways) || way.hasTag(
				"vehicle:backward") || way.hasTag(
						"vehicle:forward") || way.hasTag(
	 ...
	 *            associate a weight with every priority.
	 *            This sorted map allows subclasses to
	 *            'insert' more important priorities as well
	...
	protected void collect(ReaderWay way,
		...
		if (way.hasTag("highway", "service") && way.hasTag(
			...
			// Amandus
			String busway = way.getTag("busway");// FIXME ||
													// way.getTag("busway:right")
													// ||
													// way.getTag("busway:left");
			if (!Helper.isEmpty(busway)) {
				if ("lane".equals(busway))
					weightToPrioMap.put(10d,
							PriorityCode.PREFER.getValue());
			}
			...
			if (!Helper.isEmpty(highway)) {
				if ("motorway".equals(highway)
					...
					weightToPrioMap.put(100d,
							PriorityCode.BEST.getValue());
					...
					if (maxSpeed > 0 && maxSpeed <= 30)
						weightToPrioMap.put(120d,
								PriorityCode.REACH_DEST
										.getValue());
					else
						weightToPrioMap.put(100d,
								PriorityCode.AVOID_IF_POSSIBLE
										.getValue());
					...
					weightToPrioMap.put(40d,
							PriorityCode.AVOID_IF_POSSIBLE
									.getValue());
			} else
				weightToPrioMap.put(100d,
						PriorityCode.UNCHANGED.getValue());

			if (maxSpeed > 0) {
				// We assume that the given road segment
				// goes through a settlement.
				if (maxSpeed <= 40)
					weightToPrioMap.put(110d,
							PriorityCode.AVOID_IF_POSSIBLE
									.getValue());
				else if (maxSpeed <= 50)
					weightToPrioMap.put(110d,
